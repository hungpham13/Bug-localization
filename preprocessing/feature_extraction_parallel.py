...
class ExtractFeature(object):
    ...
    def get_prev_details(self, bug_index, src_index):
        ...
        content = " ".join(prev_bug_df['content']) if not prev_bug_df.empty else ''
        last_time = prev_bug_df['report_time'].max() if not prev_bug_df.empty else None
        length = len(prev_bug_df) if not prev_bug_df.empty else 0
        return pd.Series({'prev_content': content,
                          'last_time': last_time,
                          'length': length})
